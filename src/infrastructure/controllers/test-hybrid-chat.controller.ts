...
enum TestHybridChatFlowState {
  ...
  // AI states with phone auth (MANTÉM VALIDAÇÃO DE TELEFONE)
...
interface TestHybridChatState {
  currentState: TestHybridChatFlowState;
  data: {
    [key: string]: any;
    studentId?: string;
    userToken?: string;
...
export class TestHybridChatController {
  ...
  private async handle(message: string, state: TestHybridChatState | null): Promise<{ response: string; nextState: TestHybridChatState | null }> {
    ...
    switch (currentState) {
      ...
      case TestHybridChatFlowState.AWAITING_USER_TYPE:
      ...
      case TestHybridChatFlowState.AWAITING_STUDENT_HELP_CHOICE:
        return this.handleStudentHelpChoice(message, state!);

      case TestHybridChatFlowState.AWAITING_COORDINATOR_MENU_CHOICE:
        return this.handleCoordinatorMenuChoice(message, state!);

      case TestHybridChatFlowState.AWAITING_COORDINATOR_HELP_CHOICE:
        return this.handleCoordinatorHelpChoice(message, state!);

      case TestHybridChatFlowState.AWAITING_NEW_USER_DETAILS:
        return this.handleNewUserDetails(message, state!);

      case TestHybridChatFlowState.AWAITING_AI_PHONE:
        return await this.handleAiPhoneResponse(message, state!);

      case TestHybridChatFlowState.AI_CHAT:
        return await this.handleAiChat(message, state!);

      case TestHybridChatFlowState.AWAITING_SIMULATION_PHONE:
  ...
  private handleStart(): { response: string; nextState: TestHybridChatState } {
    const response = `[TESTE] Olá! Bem-vindo ao atendimento RADE! Para começar, me diga qual seu perfil:
...
3 - Ainda não sou usuário`;
    ...
    return {
      response,
      nextState: {
        currentState: TestHybridChatFlowState.AWAITING_USER_TYPE,
        data: {},
      },
    };
  }
...
Digite "voltar" para retornar ao menu anterior ou "sair" para encerrar.`,
    ...
    const welcomeMessage = `[TESTE] Autenticado com sucesso! Como posso ajudá-lo?\n\nDigite "voltar" para retornar ao menu principal ou "sair" para encerrar.`;
  ...
  private async handleAiChat(message: string, state: TestHybridChatState): Promise<{ response: string; nextState: TestHybridChatState }> {
    ...
    try {
      ...
      if (!result.success && result.error === 'Actor not found') {
        return {
          response: `[TESTE] Olá! Estou tendo dificuldades para acessar seus dados completos no momento.
...
9 - Encerrar atendimento`,
      nextState: {
        currentState: TestHybridChatFlowState.AWAITING_STUDENT_MENU_CHOICE,
        data,
      },
    };
  }
...
7 - Encerrar atendimento`,
      nextState: {
        currentState: TestHybridChatFlowState.AWAITING_COORDINATOR_MENU_CHOICE,
        data,
      },
    };
  }
...
O atendimento será encerrado agora. Aguarde o contato!`;
  ...
  private async buscarDadosUsuarioCompletos(cpf: string): Promise<any> {
    try {
      ...
      console.error('[TEST-HYBRID] Erro ao buscar dados do usuário:', error);
      return null;
    }
  }
  ...
  private montarContextoConversa(stateData: any): any {
    ...
    contexto.historico = [
      `Usuário: Olá`,
      `Bot: [TESTE] Olá! Para começar, me diga qual seu perfil: 1 - Sou Estudante, 2 - Sou Coordenador, 3 - Ainda não sou usuário`,
